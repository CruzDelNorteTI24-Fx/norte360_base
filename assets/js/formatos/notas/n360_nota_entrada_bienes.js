(function (window) {
    ...
    function normalize(cfg, data) {
        ...
        return C.baseNote(cfg, data, {
            ...
            pointLabel: 'Origen',
            space: 'RRHH',
            providerLabel: 'Proveedor / referencia',
            provider: '',
            documentRef: '',
            reason: '',
            withAmounts: false,
            fileName: 'norte360_formato_RE_ingreso_bienes.pdf'
        });
    }
    ...
    function drawTable(doc, note, y) {
        ...
        const x0 = 5;
        const widths = [10, 60];
        const products = (note.products && note.products.length ? note.products : [{qty: '0', description: 'Sin bienes registrados'}]);
        ...
        products.forEach(product => {
            const rowH = Math.max(3.8, doc.splitTextToSize(C.safe(product.description), 58).length * 3.8);
    ...
    function draw(doc, note, cfg) {
        ...
        y = C.drawTitleBox(doc, y, 'NOTA DE INGRESO DE BIENES ACTIVOS') + 5;
    ...
    async function generateDemo(options) {
        ...
        const cfg = Object.assign({}, options || {}, {
            noteData: window.N360NotasDemoData.get('RE', options || {})
        });
        return generate(cfg);
    }

    window.N360NotaEntradaBienes = {
        generate,
        generateDemo
    };
})(window);