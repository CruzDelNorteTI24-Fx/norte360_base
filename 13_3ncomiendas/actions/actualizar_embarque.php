...
if (!enc_schema_has_guias_norte($conn)) {
    enc_json(false, 'Falta ejecutar la migracion SQL de Guias Norte antes de actualizar el embarque.', [], 409);
}
...
try {
    ...
    if ($estado === 'EMBARCADO' && empty($guia['clm_enc_idplaca_embarque'])) {
        enc_json(false, 'Para confirmar el embarque debes seleccionar una unidad en la Guia Norte.', [], 422);
    }
