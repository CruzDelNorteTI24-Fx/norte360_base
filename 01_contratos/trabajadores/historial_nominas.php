...
// 1) Obtengo datos del conductor + licencia de la misma fila
$stmtT = $conn->prepare("
    SELECT
      clm_tra_nombres,
      clm_tra_apellidopaterno,
      clm_tra_apellidomaterno,
      clm_tra_dni,
    ...
    WHERE clm_tra_id = ?
");
$stmtT->bind_param("i", $trabid);
$stmtT->execute();
$datos = $stmtT->get_result()->fetch_assoc();
$stmtT->close();

$nombreCompleto = $datos['clm_tra_nombres']
                . ' ' . $datos['clm_tra_apellidopaterno']
                . ' ' . $datos['clm_tra_apellidomaterno'];
$dni       = $datos['clm_tra_dni'];
$edad      = calcularEdad($datos['clm_tra_fecha_nacimiento']);
$licNum    = $datos['clm_tra_nlicenciaconducir'];
$licTipo   = $datos['clm_tra_tipolicencia'];
$licCat    = $datos['clm_tra_categorialicen'];
$licExp    = $datos['clm_tra_licfecha_expedicion'];
$licReval  = $datos['clm_tra_licfecha_revaluacion'];

// 2) Ahora tu consulta de nóminas…
$stmt = $conn->prepare("
    SELECT
      n.clm_nmna_id,
      n.clm_nmna_tra_estado,
      n.clm_nmna_fechregistro,
      n.clm_nmna_fechaingrespl,
      n.clm_nmna_fechasalida,
      n.clm_nmna_com
    FROM tb_nomina n
   WHERE n.clm_nmna_trabid = ?
   ORDER BY n.clm_nmna_fechregistro DESC
");
$stmt->bind_param("i", $trabid);
$stmt->execute();
$result = $stmt->get_result();
$id       = $trabid;
// flags para mostrar o no cada foto
$hasDni1 = !empty($datos['clm_tra_dni_foto1']);
$hasDni2 = !empty($datos['clm_tra_dni_foto2']);
$hasLic1 = !empty($datos['clm_tra_lic_foto1']);
$hasLic2 = !empty($datos['clm_tra_lic_foto2']);
?>
...
<head>
    <meta charset="UTF-8">
    <title>Historial de Nóminas | Norte 360°</title>
...
<div class="menu-lateral" id="menuLateral">
  ...
  <ul>
    ...
    <li><a href="ver_personal.php">Trabajadores</a></li>
    <li><a href="ver_licencias.php">Ver Licencias</a></li>
    <li><a href="ver_cumpleanos.php">Ver Cumpleaños</a></li>
    <li><a href="ver_cargos.php">Ver Cargos</a></li>
    <li><a href="ver_emergencia.php">Ver Contactos Emergencia</a></li>
...
<div style="text-align: right; margin: 20px 0;">
  <a href="create_nomina.php" class="btn-validar" style="padding:10px 20px;">
    <i class="fa-solid fa-plus"></i> Agregar Nómina
  </a>
</div>
    <!-- bloque datos y fotos -->
    <div class="datos-conductor" style="max-width:800px;margin:20px auto;padding:15px;background:#fff;border-radius:8px;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
      <h3>Conductor</h3>
      <ul style="list-style:none;padding:0;margin:0 0 10px 0;">
        <li><strong>Nombre:</strong> <?= htmlspecialchars($nombreCompleto) ?></li>
        <li><strong>DNI:</strong> <?= htmlspecialchars($dni) ?></li>
        <li><strong>Edad:</strong> <?= $edad ?> años</li>
      </ul>

      <!-- aquí las fotos -->
<div class="fotos-conductor">
  <?php if($hasDni1): ?>
    <div class="foto-block">
      <img src="ver_archivo.php?id=<?= $id ?>&campo=clm_tra_dni_foto1" alt="DNI Foto 1">
      <div class="caption">DNI Foto 1</div>
    </div>
  <?php endif; ?>

  <?php if($hasDni2): ?>
    <div class="foto-block">
      <img src="ver_archivo.php?id=<?= $id ?>&campo=clm_tra_dni_foto2" alt="DNI Foto 2">
      <div class="caption">DNI Foto 2</div>
    </div>
  <?php endif; ?>

  <?php if($hasLic1): ?>
    <div class="foto-block">
      <img src="ver_archivo.php?id=<?= $id ?>&campo=clm_tra_lic_foto1" alt="Licencia Foto 1">
      <div class="caption">Licencia Foto 1</div>
    </div>
  <?php endif; ?>

  <?php if($hasLic2): ?>
    <div class="foto-block">
      <img src="ver_archivo.php?id=<?= $id ?>&campo=clm_tra_lic_foto2" alt="Licencia Foto 2">
      <div class="caption">Licencia Foto 2</div>
    </div>
  <?php endif; ?>
</div>
      ...
      <ul style="list-style:none;padding:0;margin:0;">
        <li><strong>Número:</strong> <?= htmlspecialchars($licNum) ?: '<em>--</em>' ?></li>
        <li><strong>Tipo:</strong>  <?= htmlspecialchars($licTipo) ?: '<em>--</em>' ?></li>
        <li><strong>Categoría:</strong> <?= htmlspecialchars($licCat) ?: '<em>--</em>' ?></li>
        <li><strong>Expedición:</strong> <?= htmlspecialchars($licExp) ?: '<em>--</em>' ?></li>
        <li><strong>Revalidación:</strong> <?= htmlspecialchars($licReval) ?: '<em>--</em>' ?></li>
      </ul>
    </div>
<h2>Historial de Nóminas de <strong><?= htmlspecialchars($nombreCompleto) ?></strong></h2>
...
<button id="descargarPDF" class="btn-cv-profesional" style="margin-bottom:18px;">
  ...
  <table>
    ...
    <?php while($row = $result->fetch_assoc()): 
        $salida = $row['clm_nmna_fechasalida'] ?: 'Vigente';
    ?>
      <tr>
        <td><?= $row['clm_nmna_id'] ?></td>
        <td>
          <?= $row['clm_nmna_tra_estado']
                ? '<span style="color:green">Activo</span>'
                : '<span style="color:red">Inactivo</span>' ?>
        </td>
        <td><?= $row['clm_nmna_fechregistro'] ?></td>
        <td><?= $row['clm_nmna_fechaingrespl'] ?></td>
        <td><?= $salida ?></td>
        <td><?= nl2br(htmlspecialchars($row['clm_nmna_com'])) ?></td>
      </tr>
    <?php endwhile; ?>
    </tbody>
  </table>
</div>
  <p><a href="ver_listanomina.php">« Volver a lista</a></p>
