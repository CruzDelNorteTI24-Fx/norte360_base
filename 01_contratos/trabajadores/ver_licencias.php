...
function calcularDias($fecha) {
    $hoy = new DateTime();
    $expedicion = new DateTime($fecha);
    $diff = $hoy->diff($expedicion);
    return $diff->days;
}
...
if ($search != '') {
    $sql = "SELECT SQL_CALC_FOUND_ROWS clm_tra_id, clm_tra_nombres, clm_tra_dni, clm_tra_nlicenciaconducir, clm_tra_licfecha_expedicion
            FROM tb_trabajador
            WHERE (clm_tra_nombres LIKE ? OR clm_tra_dni LIKE ? OR clm_tra_nlicenciaconducir LIKE ?)
            ORDER BY clm_tra_licfecha_expedicion
...
<h2>🚘 Licencias de Conducir Registradas</h2>

<form class="search-form" onsubmit="return false;">
  <input type="text" id="buscador" placeholder="Buscar por nombre, DNI o Número de Licencia ..." class="search-input">
</form>
...
<table>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><?= htmlspecialchars($row['clm_tra_nlicenciaconducir']) ?></td>
            <td><?= htmlspecialchars($row['clm_tra_licfecha_expedicion']) ?></td>
            <td><?= calcularDias($row['clm_tra_licfecha_expedicion']) ?> días</td>
        </tr>
        <?php endwhile; ?>
    </tbody>
</table>
