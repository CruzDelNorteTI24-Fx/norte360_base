...
if ($search != '') {
    $sql = "
      ...
      FROM tb_nomina n
      JOIN tb_trabajador t ON t.clm_tra_id = n.clm_nmna_trabid
      WHERE t.clm_tra_nombres LIKE ? 
         OR t.clm_tra_dni     LIKE ?
      ORDER BY n.clm_nmna_fechaingrespl DESC
...
<head>
    <meta charset="UTF-8">
    <title>Crear Nóminas | Norte 360°</title>
...
<h2>Lista de Nóminas</h2>
    ...
    <h2>✔️ Crear Nueva Nómina</h2>
    <form method="post" action="guardar_nomina.php" enctype="multipart/form-data">
      <div class="campo-form">
        <label for="nomina_conductor">Conductor</label>
        <select name="clm_nmna_trabid" id="nomina_conductor" required>
          <option value="">-- Selecciona conductor --</option>
          <?php
            // En lugar de la consulta simple, usamos un LEFT JOIN con un sub‐query
            $rs = $conn->query("
            SELECT
                t.clm_tra_id,
                t.clm_tra_nombres
            ...
            LEFT JOIN (
                /* obtenemos el estado de la nómina más reciente de cada trabajador */
                SELECT n1.clm_nmna_trabid, n1.clm_nmna_tra_estado
                FROM tb_nomina n1
                INNER JOIN (
                SELECT clm_nmna_trabid, MAX(clm_nmna_fechaingrespl) AS max_ing
                FROM tb_nomina
                GROUP BY clm_nmna_trabid
                ) m ON n1.clm_nmna_trabid = m.clm_nmna_trabid
                AND n1.clm_nmna_fechaingrespl = m.max_ing
            ) lastn ON t.clm_tra_id = lastn.clm_nmna_trabid
            WHERE t.clm_tra_tipo_trabajador = 'Conductor'
                /* sólo los que no tengan nómina activa en su último ingreso */
                AND ( lastn.clm_nmna_tra_estado IS NULL OR lastn.clm_nmna_tra_estado = 2 )
            ORDER BY t.clm_tra_nombres
            ");

            while($c = $rs->fetch_assoc()){
            echo "<option value=\"{$c['clm_tra_id']}\">"
                .htmlspecialchars($c['clm_tra_nombres'])
                ."</option>";
            }
          ?>
        </select>
      ...
      <div class="campo-form">
        <label for="nomina_fechaing">Fecha de Ingreso</label>
        <input type="date" name="clm_nmna_fechaingrespl" id="nomina_fechaing" required>
      </div>
      <div class="campo-form">
        <label for="nomina_doc">Documento (PDF)</label>
        <input type="file" name="clm_nmna_doc" id="nomina_doc" accept=".pdf" required>
      </div>
      <div class="campo-form">
        <label for="nomina_com">Comentario</label>
        <textarea name="clm_nmna_com" id="nomina_com" rows="4"></textarea>
      </div>
      <button type="submit" class="btn-validar">
        <i class="fa-solid fa-floppy-disk"></i> Guardar Nómina
      </button>
      <a href="ver_listanomina.php" style="margin-left: 10px; text-decoration:none; color:#2980b9;">Cancelar</a>
    </form>
