...
if ($search != '') {
    $sql = "SELECT SQL_CALC_FOUND_ROWS clm_tra_id, clm_tra_nombres, clm_tra_dni, clm_tra_sexo, clm_tra_fecha_nacimiento
            ...
            WHERE (clm_tra_nombres LIKE ? OR clm_tra_dni LIKE ?)
              AND clm_tra_tipo_trabajador = 'Conductor'   -- <-- filtro agregado
            ORDER BY clm_tra_id
    ...
    $sql = "SELECT SQL_CALC_FOUND_ROWS clm_tra_id, clm_tra_nombres, clm_tra_dni, clm_tra_sexo, clm_tra_fecha_nacimiento
            FROM tb_trabajador
            WHERE clm_tra_tipo_trabajador = 'Conductor'   -- <-- filtro agregado
            ORDER BY clm_tra_id
            LIMIT $limit OFFSET $offset";
    $stmt = $conn->prepare($sql);
}
...
<head>
    <meta charset="UTF-8">
    <title>Lista de Conductores | Norte 360°</title>
