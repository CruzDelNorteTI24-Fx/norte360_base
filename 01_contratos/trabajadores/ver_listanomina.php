...
<head>
    <meta charset="UTF-8">
    <title>Histórico de Nóminas | Norte 360°</title>
...
<?php while($row = $resultado->fetch_assoc()): ?>
    <?php
      ...
      $fechaSalida = $row['clm_nmna_fechasalida']
        ? $row['clm_nmna_fechasalida']
        : 'Vigente';
    ?>
    <tr>
      ...
      <td>
        <?= $row['clm_nmna_tra_estado']==1
              ? '<span style="color:green">Activo</span>'
              : '<span style="color:red">Inactivo</span>' ?>
      </td>
      <td><?= $row['clm_nmna_fechregistro'] ?></td>
      <td><?= $row['clm_nmna_fechaingrespl'] ?></td>
      <!-- 2) Aquí sí lo mostramos -->
      <td><?= $fechaSalida ?></td>
      <td>
        <a 
          href="historial_nominas.php?trabid=<?= $row['clm_nmna_trabid'] ?>" 
          class="btn-ver"
        >
          Ver historial
        </a>
