...
if ($search != '') {
    $sql = "SELECT SQL_CALC_FOUND_ROWS clm_tra_id, clm_tra_nombres, clm_tra_dni, clm_tra_sexo, clm_tra_fecha_nacimiento, clm_tra_tipo_trabajador, clm_tra_cargo, clm_tra_nlicenciaconducir, clm_tra_celular, clm_tra_identrevista
            FROM tb_trabajador
...
.btn-accion i { margin-right: 6px; }
.btn-accion:hover { filter: brightness(0.92); }
.btn-detalles { background: #2980b9; }
.btn-planillas { background: #16a085; }
.badge-vigente{
  display:inline-block;
  margin-left:8px;
  padding:2px 8px;
...
.btn-sec{
  background:#7f8c8d; color:#fff; border:0; border-radius:8px; padding:10px 14px; cursor:pointer; font-weight:600;
}
.btn-prim{
  background:#2980b9; color:#fff; border:0; border-radius:8px; padding:10px 14px; cursor:pointer; font-weight:700;
}
.btn-danger{
...
<tbody>
    <?php foreach($trabajadores as $row): ?>
      <tr>
        <td><?= htmlspecialchars($row['clm_tra_id']) ?></td>
        <td><?= htmlspecialchars($row['clm_tra_nombres']) ?></td>
        <td><?= htmlspecialchars($row['clm_tra_dni']) ?></td>
        <td><?= htmlspecialchars($row['clm_tra_sexo']) ?></td>
        <td><?= htmlspecialchars($row['clm_tra_fecha_nacimiento']) ?></td>
        <td><?= $row['edad_actual'] ?> años</td>
        <td><?= htmlspecialchars($row['clm_tra_tipo_trabajador']) ?></td>
        <td><?= htmlspecialchars($row['clm_tra_cargo']) ?></td>

        <!-- Licencia de Conducir -->
        <td>
          <?php
            if (empty($row['clm_tra_nlicenciaconducir'])) {
                // Si no hay licencia
                echo '<span style="color:#888;font-style:italic;"><i class="fa-solid fa-circle-exclamation"></i> Sin licencia registrada</span>';
            } else {
                // Licencia presente
                echo htmlspecialchars($row['clm_tra_nlicenciaconducir']);
            }
          ?>
        </td>

        <td><?= htmlspecialchars($row['clm_tra_celular']) ?></td>

        <!-- Origen de Entrevista -->
        <td>
          <?php
            if (empty($row['clm_tra_identrevista'])) {
                echo '<span style="color:#888;"><i class="fa-solid fa-ban"></i> Directo</span>';
            } else {
                echo 'N° ' . htmlspecialchars($row['clm_tra_identrevista']);
            }
          ?>
        </td>
        ...
        <td>
          <?php
            $info = $mapa_contratos[(int)$row['clm_tra_id']] ?? ['tabla_estado' => 'INACTIVO'];
            $estadoTabla = strtoupper($info['tabla_estado'] ?? 'INACTIVO');

            if ($estadoTabla === 'ACTIVO') {
                echo '<span style="color:#27ae60;font-weight:bold;">ACTIVO</span>';
            } else {
                echo '<span style="color:#c0392b;font-weight:bold;">INACTIVO</span>';
            }
          ?>
        </td>
        ...
        <td class="col-acciones">
          <a href="detalle_trabajador.php?id=<?= (int)$row['clm_tra_id'] ?>" 
            class="btn-accion btn-detalles" title="Ver detalles">
            <i class="fa-solid fa-magnifying-glass"></i> Detalles
          </a>

          <button type="button"
                  class="btn-accion btn-planillas"
                  title="Ver historial de planillas"
                  onclick="abrirModalPlanillas(<?= (int)$row['clm_tra_id'] ?>,'<?= htmlspecialchars($row['clm_tra_nombres'], ENT_QUOTES) ?>')">
            <i class="fa-solid fa-file-contract"></i> Planillas
          </button>
        </td>
...
<div id="modalPlanillas" class="modal">
  <div class="modal-content">
    <span class="cerrar" onclick="cerrarModalPlanillas()">&times;</span>
    <h3 id="tituloPlanillas" style="margin-top:0;">Historial de Planillas</h3>
    <div id="contenedorPlanillas">
      <p>Cargando…</p>
    </div>
  </div>
</div>
...
<script>
function abrirModalPlanillas(id, nombre) {
  const modal  = document.getElementById('modalPlanillas');
  const cont   = document.getElementById('contenedorPlanillas');
  const titulo = document.getElementById('tituloPlanillas');

  titulo.textContent = 'Historial de Planillas — ' + nombre + ' (ID ' + id + ')';
  cont.innerHTML = '<p>Cargando…</p>';
  modal.style.display = 'block';

  fetch('../api/planillas_historial.php?id=' + encodeURIComponent(id), { credentials: 'same-origin' })
    .then(r => r.json())
    .then(j => {
      if (!j.ok) {
        cont.innerHTML = '<p style="color:#c0392b;">' + (j.error || 'No se pudo cargar el historial.') + '</p>';
        return;
      }

      let html = '';
      if (!j.has_vigente) {
        html += '<div class="alerta-sin-vigente" style="padding:10px 14px;border-radius:8px;background:#fdecea;color:#c0392b;margin-bottom:10px;font-weight:600;">' +
                  'NO TIENE CONTRATO VIGENTE' +
                '</div>';
      }

      if (!j.data || j.data.length === 0) {
        html += '<p class="no-image">Sin registros de planilla.</p>';
      } else {
        html += '<div class="tabla-scroll-x"><table><thead><tr>' +
                  '<th>#</th><th>Fecha de Ingreso</th><th>Tipo</th><th>Estado</th><th>Fecha salida</th><th>Documento</th><th>Comentario</th>' +
                '</tr></thead><tbody>';

        j.data.forEach((it, idx) => {
          const estadoClase = (it.tabla_estado === 'ACTIVO') ? 'estado-activo' : 'estado-inactivo';
          const badge = it.is_vigente ? ' <span class="badge-vigente">VIGENTE</span>' : '';
          html += '<tr>' +
                    '<td>' + (idx + 1) + '</td>' +
                    '<td>' + (it.clm_pl_fechaingrespl || '') + '</td>' +
                    '<td>' + (it.clm_pl_tipo || '') + '</td>' +
                    '<td class="' + estadoClase + '">' + it.tabla_estado + badge + '</td>' +
                    '<td>' + (it.clm_pl_fechasalida || '') + '</td>' +
                    '<td>' + (it.clm_pl_doc ? ('<a href="' + it.clm_pl_doc + '" target="_blank">Ver</a>') : '<span style="color:#888;font-style:italic;">—</span>') + '</td>' +
                    '<td>' + (it.clm_pl_com ? it.clm_pl_com : '') + '</td>' +
                  '</tr>';
        });
      ...
      html += '<button class="btn-sec" onclick="cerrarModalPlanillas()">Cerrar</button>';
      if (j.has_vigente) {
        html += '<button class="btn-danger" onclick="darDeBaja(' + id + ')">Dar de baja</button>';
      } else {
        html += '<button class="btn-prim" onclick="contratar(' + id + ')">Contratar</button>';
      }
      html += '</div>';
    ...
    .catch(() => {
      cont.innerHTML = '<p style="color:#c0392b;">Error de red al cargar el historial.</p>';
    });
}

function cerrarModalPlanillas() {
  document.getElementById('modalPlanillas').style.display = 'none';
}
...
async function postJSON(url, data) {
  ...
  try { json = JSON.parse(text); }
    ...
    // Muestra el HTML/fatal del servidor para depurar
...
// Placeholders de acción (ajusta a tus rutas reales)
async function darDeBaja(trabId) {
  if (!confirm('¿Dar de baja el contrato vigente?')) return;
  try {
    await postJSON('../api/planillas_baja.php', { trabid: trabId });
    alert('Baja realizada.');
    const t = document.getElementById('tituloPlanillas').textContent;
    const m = t.match(/—\s*(.+)\s*\(ID/); const nombre = m ? m[1].trim() : 'Trabajador';
    abrirModalPlanillas(trabId, nombre);
    // location.reload(); // si quieres refrescar toda la grilla
  } catch (e) { alert('Error: ' + e.message); }
}
async function contratar(trabId) {
  const tipo = prompt('Tipo de contrato: 1=PLANILLA, 2=RECIBO POR HONORARIOS', '1');
  if (tipo === null) return;
  const tipoInt = parseInt(tipo,10);
  if (![1,2].includes(tipoInt)) { alert('Tipo inválido'); return; }
  try {
    await postJSON('../api/planillas_contratar.php', { trabid: trabId, tipo: tipoInt });
