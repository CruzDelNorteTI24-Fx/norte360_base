...
$sql = "SELECT d.clm_doc_iddocumento, d.clm_doc_fecharegistro, d.clm_doc_observaciones, 
               d.clm_doc_archivo, d.clm_doc_nombre_archivo,
               t.nombre_tipo,
...
<head>
    <meta charset="UTF-8">
    <title>Detalle de Documentación | Norte 360°</title>
...
<div class="seccion-formulario">
  ...
  <div class="grid-campos">
    <p><span>Tipo:</span> <?= htmlspecialchars($doc['nombre_tipo']) ?></p>
    ...
    <p><span>Observaciones:</span> <?= htmlspecialchars($doc['clm_doc_observaciones']) ?: 'Sin observaciones' ?></p>
  </div>
</div>


<?php if (!empty($doc['clm_doc_archivo'])): ?>
  <div class="seccion-formulario">
    <h3>Archivo Subido</h3>
    <?php
      $extension = strtolower(pathinfo($doc['clm_doc_nombre_archivo'], PATHINFO_EXTENSION));
      $ruta = "ver_archivo.php?id=" . $doc['clm_doc_iddocumento'];
      $esImagen = in_array($extension, ['jpg', 'jpeg', 'png', 'webp']);
      $esPDF = $extension === 'pdf';
    ?>
    <?php if ($esImagen): ?>
      <div class="img-block">
        <img src="<?= $ruta ?>" alt="Imagen del documento">
      </div>
    <?php elseif ($esPDF): ?>
      <iframe src="<?= $ruta ?>" style="width: 100%; height: 480px; border: 1px solid #ccc; border-radius: 8px;"></iframe>
    <?php else: ?>
      <p><strong>Archivo:</strong> <?= htmlspecialchars($doc['clm_doc_nombre_archivo']) ?></p>
      <a href="<?= $ruta ?>" target="_blank" class="btn-cv-profesional">📥 Descargar archivo</a>
    <?php endif; ?>
  </div>
<?php else: ?>
  <p class="no-image">No se ha subido ningún archivo.</p>
<?php endif; ?>



<a href="ver.php" class="volver-btn">⬅ Volver a la vista general</a>
</div>
