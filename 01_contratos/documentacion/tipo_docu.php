...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["nueva_clasificacion"])) {
    ...
    $descClasificacion = trim($_POST["desc_clasificacion"]);
    if ($nuevaClasificacion !== "") {
        $stmt = $conn->prepare("INSERT INTO tb_clasificacion_documento (clm_clas_name, clm_clas_descp) VALUES (?, ?)");
        $stmt->bind_param("ss", $nuevaClasificacion, $descClasificacion);
        $stmt->execute();
        $stmt->close();
        $_SESSION['exito'] = true;
        header("Location: tipo_docu.php");
        exit();
    }
}
...
if ($resultClas) {
    while ($row = $resultClas->fetch_assoc()) {
        $clasificaciones[] = $row;
    }
    $resultClas->free();
}
...
$tipos = [];
$result = $conn->query("
    SELECT td.id_tipo_documento, td.nombre_tipo, cd.clm_clas_name 
    FROM tb_tipo_documento td
    JOIN tb_clasificacion_documento cd ON td.clm_tipo_idclas = cd.clm_clas_id
...
<head>
    <meta charset="UTF-8">
    <title>Agregar Tipo Documento | Norte 360°</title>
  ...
  <div class="section-flex">
            ...
            <div class="listado">
              ...
              <div class="item-listado">
                <strong><?= htmlspecialchars($clas['clm_clas_name']) ?></strong>
                <span><?= htmlspecialchars($clas['clm_clas_descp'] ?? 'Sin descripción') ?></span>
              </div>
              <?php endforeach; ?>
            </div>
          ...
          <form method="POST" class="formulario-entrevista">
                ...
                <div class="campo-form">
                  ...
                  <select name="id_clasificacion" id="id_clasificacion" required class="input-evaluacion">
                    ...
                    <?php foreach ($clasificaciones as $clas): ?>
                      <option value="<?= $clas['clm_clas_id'] ?>"><?= htmlspecialchars($clas['clm_clas_name']) ?></option>
                    <?php endforeach; ?>
                  </select>
                </div>
