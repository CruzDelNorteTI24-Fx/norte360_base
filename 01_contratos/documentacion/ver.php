...
$documentos = obtenerDocumentacion($conn);
$conn->close();
?>
...
<head>
    <meta charset="UTF-8">
    <title>Ver Documentación | Norte 360°</title>
...
.selector-vista input[type="radio"] {
  display: none;
}
...
.selector-vista input[type="radio"]:checked + label {
  background: #3498db;
  color: white;
...
.document-card .doc-detalle h4 {
  margin: 0 0 12px 0;
  font-size: 18px;
  color: #2c3e50;
}

.document-card .doc-detalle p {
  margin: 4px 0;
  font-size: 15px;
  color: #555;
}

.document-card .doc-detalle p span {
  font-weight: bold;
  color: #2980b9;
  margin-right: 6px;
  text-align: center;
...
.document-card {
  text-align: left; /* 👈 fuerza alineación izquierda del contenido */
}

.document-card .doc-detalle {
  display: flex;
  flex-direction: column;
...
<div class="main-content">
  ...
  <div style="max-width: 500px; margin: 0 auto 30px auto;">
    <input type="text" id="filtroDocumento" class="input-evaluacion" placeholder="🔍 Buscar por tipo, nombre, DNI u observaciones...">
  </div>

  <div class="selector-vista">
    <input type="radio" id="vista_tabla" name="vista" value="tabla" checked onclick="cambiarVistaConAnimacion('tabla')">
    <label for="vista_tabla">📄 Tabla</label>

    <input type="radio" id="vista_cards" name="vista" value="cards" onclick="cambiarVistaConAnimacion('cards')">
    <label for="vista_cards">🧾 Detallado</label>
  </div>



  <div id="vista-tabla" class="tabla-contenedor">
    <table>
      <thead>
        <tr>
      ...
      <tbody>
        <?php if (count($documentos) === 0): ?>
          <tr><td colspan="5" style="text-align:center;">No hay documentos registrados.</td></tr>
        <?php else: ?>
          <?php foreach ($documentos as $doc): ?>
            <tr>
              <td><?= htmlspecialchars($doc['clm_tra_nombres']) ?></td>
              <td><?= htmlspecialchars($doc['clm_tra_dni']) ?></td>
              <td><?= htmlspecialchars($doc['nombre_tipo']) ?></td>
              <td><?= htmlspecialchars($doc['clm_doc_fecharegistro']) ?></td>
              <td><?= htmlspecialchars($doc['clm_doc_observaciones']) ?></td>
            </tr>
          <?php endforeach; ?>
        <?php endif; ?>
      </tbody>
...
<div id="vista-cards" class="catalogo-container" style="display:none; flex-direction: column; gap: 40px;">
  <?php
    // Agrupar documentos por tipo
    $documentosPorTipo = [];
    foreach ($documentos as $doc) {
        $tipo = $doc['nombre_tipo'];
  ...
  <?php foreach ($documentosPorTipo as $tipo => $docs): ?>
    <div class="grupo-documentos" style="margin-bottom: 40px;">
      <h3 style="margin: 20px 0 15px; color: #2c3e50; border-left: 5px solid #3498db; padding-left: 12px; font-size: 20px;">
        📁 <?= htmlspecialchars($tipo) ?>
      </h3>
      <div class="catalogo-container">
        <?php foreach ($docs as $doc): ?>
          <div class="product-card document-card">
            <div class="doc-icono">
              <img src="../../img/icons/paper.png" alt="Documento">
            </div>
            <div class="doc-detalle">
              <h4><?= htmlspecialchars($doc['nombre_tipo']) ?></h4>
              <p><span>Nombre:</span> <?= htmlspecialchars($doc['clm_tra_nombres']) ?></p>
              <p><span>DNI:</span> <?= htmlspecialchars($doc['clm_tra_dni']) ?></p>
              <p><span>Registro:</span> <?= htmlspecialchars($doc['clm_doc_fecharegistro']) ?></p>
              <p><span>Observaciones:</span> <?= htmlspecialchars($doc['clm_doc_observaciones']) ?></p>
              <a href="detalle.php?id=<?= $doc['clm_doc_iddocumento'] ?>" class="btn-cv-profesional" style="margin-top:15px;">Ver documento</a>
            </div>
          </div>
        <?php endforeach; ?>
      </div>
...
<script>
function cambiarVistaConAnimacion(vista) {
  const tabla = document.getElementById('vista-tabla');
  const cards = document.getElementById('vista-cards');

  // Oculta ambas temporalmente
  tabla.style.opacity = '0';
  cards.style.opacity = '0';

  setTimeout(() => {
    tabla.style.display = vista === 'tabla' ? 'flex' : 'none';
    cards.style.display = vista === 'cards' ? 'grid' : 'none';
...
document.getElementById('filtroDocumento').addEventListener('input', function () {
  ...
  // Filtrar vista tabla
  const filas = document.querySelectorAll('#vista-tabla tbody tr');
  filas.forEach(fila => {
  ...
  // Filtrar vista cards
  const grupos = document.querySelectorAll('#vista-cards .grupo-documentos');
  grupos.forEach(grupo => {
    let algunaVisible = false;

    const tarjetas = grupo.querySelectorAll('.document-card');
