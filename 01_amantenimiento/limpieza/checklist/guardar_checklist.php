...
if ($_SERVER["REQUEST_METHOD"] == "POST") {    
  ...
  $id_tipo_checklist = (int)$_POST['id_tipo_checklist'];
  ...
  if ($id_version_checklist !== null && n360_cv_checklist_version_ready($conn)) {
    $stmt = $conn->prepare("INSERT INTO tb_checklist_limpieza (clm_checklist_id_bus, clm_checklist_fecha, clm_checklist_hora, clm_checklist_responsable, clm_checklist_idpersonaregistra, clm_checklist_observaciones, clm_checklist_corr, clm_checklist_idtipo, clm_checklist_idversion) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("isssissii", $bus, $fecha, $hora, $responsable, $id_registra, $observaciones, $clm_corr, $id_tipo_checklist, $id_version_checklist);
  } else {
