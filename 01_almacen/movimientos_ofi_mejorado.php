...
/* =========================================================
   MOVIMIENTOS OFI · UI PRO
   Filtros compactos, íconos proporcionales y tabla amplia
   ========================================================= */
:root{
  --mov-bg:#eef2f7;
  --mov-ink:#0f172a;
...
.filters-toolbar{
  position:relative;
  background:linear-gradient(135deg,#162233 0%, #1f2d3d 100%);
  border:1px solid rgba(14,165,233,.55);
  border-radius:18px;
  padding:18px;
  margin:18px auto 16px;
  color:#e5e7eb;
  box-shadow:0 12px 26px rgba(15,23,42,.16), inset 0 1px 0 rgba(255,255,255,.04);
  overflow:hidden;
}
.filters-toolbar::before{
  content:"";
  position:absolute;
  inset:0 0 auto 0;
  height:3px;
  background:linear-gradient(90deg,#0ea5e9,#22c55e,#facc15,#0ea5e9);
  opacity:.95;
}
...
.filter-field{ grid-column:span 3; min-width:0; }
.filter-field.wide{ grid-column:span 4; }
.filter-field.type-field{ grid-column:span 6; }
.filter-field.quick-field{ grid-column:span 8; }
...
.filters-toolbar input.form-control:focus,
.filters-toolbar select.form-select:focus{
  background:#0d1626;
  border-color:#38bdf8;
  box-shadow:0 0 0 3px rgba(14,165,233,.16);
}
.filters-toolbar input::placeholder{ color:#94a3b8; }
.filters-toolbar input[type="date"]::-webkit-calendar-picker-indicator{
...
.type-btn i{ font-size:.95rem; line-height:1; }
.btn-check:checked + .type-btn{
  background:#0ea5e9 !important;
  border-color:#0ea5e9 !important;
  color:#06121f !important;
  box-shadow:0 8px 16px rgba(14,165,233,.24);
}
...
.btn-filter-primary:hover{ background:#38bdf8; border-color:#38bdf8; color:#07111f; }
.btn-filter-secondary{
  background:#0b1220;
  border:1px solid #2a3a4f;
  color:#e5e7eb;
}
.btn-filter-secondary:hover{ background:#111c2e; color:#fff; }
...
.actionbar{
  ...
  gap:12px;
  color:#e5e7eb;
  box-shadow:0 10px 24px rgba(15,23,42,.14), inset 0 1px 0 rgba(255,255,255,.03);
}
.actionbar-left,
.actionbar-right{
  display:flex;
...
.actionbar-right{ justify-content:flex-end; }
...
.btn-solid{ background:#0ea5e9; border-color:#0ea5e9; color:#07111f; }
.btn-solid:hover{ background:#38bdf8; border-color:#38bdf8; color:#07111f; }
.btn-soft{ background:#111827; color:#e5e7eb; border-color:#26364b; }
.btn-soft:hover{ background:#1b2433; color:#fff; }
.btn-ghost{ background:transparent; color:#dbeafe; border-color:#26364b; }
.btn-ghost:hover{ background:#111827; color:#fff; }
.stat-pill{
  min-height:34px;
  display:inline-flex;
  align-items:center;
...
.kpi-card{
  ...
  border-radius:16px;
  padding:13px;
  box-shadow:0 10px 22px rgba(15,23,42,.14), inset 0 1px 0 rgba(255,255,255,.03);
  display:flex;
  gap:12px;
  align-items:center;
  min-width:0;
...
.kpi-card .meta{ min-width:0; flex:1; }
.kpi-card .meta .label{ color:#cbd5e1; font-size:.78rem; font-weight:800; }
.kpi-card .meta .value{ font-weight:900; font-size:1.2rem; line-height:1.15; margin-top:2px; }
.kpi-card .meta .sub{ color:#94a3b8; font-size:.76rem; margin-top:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.kpi-total{ border-color:#475569; }
.kpi-in{ border-color:#15803d; }
.kpi-out{ border-color:#b91c1c; }
.kpi-inv{ border-color:#d97706; }
.kpi-net{ border-color:#0891b2; }
...
.table-shell{ position:relative; margin-top:0; }
.slide-ctrl{
  position:absolute;
  top:50%;
...
.slide-left{ left:-10px; border-radius:0 14px 14px 0; }
.slide-right{ right:-10px; border-radius:14px 0 0 14px; }
.table-wrap{
  overflow:auto;
  padding-bottom:8px;
...
.badge{ padding:.34rem .58rem; border-radius:999px; font-weight:800; font-size:.72rem; }
.b-in{ background:#dcfce7; color:#166534; border:1px solid #bbf7d0; }
.b-out{ background:#fee2e2; color:#991b1b; border:1px solid #fecaca; }
.b-inv{ background:#fff7ed; color:#9a3412; border:1px solid #fed7aa; }
.b-estado{ background:#eef2ff; color:#3730a3; border:1px solid #c7d2fe; }
.obs{ max-width:440px; white-space:nowrap; text-overflow:ellipsis; overflow:hidden; }
.btn-note{
  height:32px;
  display:inline-flex;
  align-items:center;
...
.pagination a,.pagination span{ padding:7px 11px; border-radius:9px; font-weight:800; text-decoration:none; }
.pagination a{ background:#0ea5e9; color:#07111f; }
.pagination a:hover{ background:#38bdf8; color:#07111f; }
.pagination .curr{ color:#0f172a; background:#e0f2fe; border:1px solid #bae6fd; }
.muted{ color:#667085; }

@media(max-width:1300px){
  .filters-grid{ grid-template-columns:repeat(6, minmax(0,1fr)); }
  .filter-field,.filter-field.wide{ grid-column:span 3; }
  .filter-field.type-field,.filter-field.quick-field,.filter-actions{ grid-column:span 6; }
  .kpis-pro{ grid-template-columns:repeat(3, minmax(170px,1fr)); }
}
@media(max-width:900px){
  .main-content{ margin-left:0; padding:22px 14px; }
  .filters-grid{ grid-template-columns:1fr; }
  .filter-field,.filter-field.wide,.filter-field.type-field,.filter-field.quick-field,.filter-actions{ grid-column:span 1; }
  .actionbar{ align-items:flex-start; flex-direction:column; }
  .actionbar-left,.actionbar-right{ width:100%; justify-content:flex-start; }
  .kpis-pro{ grid-template-columns:1fr 1fr; }
}
@media(max-width:560px){
  .kpis-pro{ grid-template-columns:1fr; }
...
<main>
  ...
  <p class="page-subtitle">Consulta operativa de entradas, salidas e inventariados con filtros y exportación.</p>
...
<form id="filtrosMov" class="filters-toolbar" method="get">
  <div class="title">
    ...
    <span class="filter-hint">Filtra por periodo, tipo, producto, placa o etiqueta</span>
  </div>
  ...
  <div class="filters-grid">
    ...
    <div class="filter-field">
      ...
      <div class="input-wrap date-field">
        <span class="field-ico"><i class="bi bi-calendar-event"></i></span>
    ...
    <div class="filter-field">
      ...
      <div class="input-wrap date-field">
        <span class="field-ico"><i class="bi bi-calendar-check"></i></span>
    ...
    <div class="filter-field">
      ...
      <div class="input-wrap">
        <span class="field-ico"><i class="bi bi-box-seam"></i></span>
    ...
    <div class="filter-field wide">
      ...
      <div class="input-wrap">
        <span class="field-ico"><i class="bi bi-search"></i></span>
        <input type="text" name="buscar" value="<?=$buscar?>" class="form-control" placeholder="Producto, código, documento, RUC o factura">
      </div>
    </div>
    ...
    <div class="filter-field type-field">
      ...
      <div class="type-group" role="group" aria-label="Tipo movimiento">
        <?php $is = fn($v)=> $tipo===$v ? 'checked' : ''; ?>
        <input type="radio" class="btn-check" name="tipo" id="tAll" value=""  <?= $tipo===''?'checked':''; ?>>
        <label class="type-btn" for="tAll"><i class="bi bi-sliders"></i> Todos</label>

        <input type="radio" class="btn-check" name="tipo" id="tIn" value="ENTRADA" <?= $is('ENTRADA'); ?>>
        <label class="type-btn" for="tIn"><i class="bi bi-box-arrow-in-down"></i> Entrada</label>

        <input type="radio" class="btn-check" name="tipo" id="tOut" value="SALIDA" <?= $is('SALIDA'); ?>>
        <label class="type-btn" for="tOut"><i class="bi bi-box-arrow-up"></i> Salida</label>

        <input type="radio" class="btn-check" name="tipo" id="tInv" value="INVENTARIADO" <?= $is('INVENTARIADO'); ?>>
        <label class="type-btn" for="tInv"><i class="bi bi-clipboard-check"></i> Inventariado</label>
      </div>
    </div>

    <!-- Placa -->
    <div class="filter-field">
      <label class="field-label">Placa / Bus</label>
      <div class="input-wrap">
        <span class="field-ico"><i class="bi bi-bus-front"></i></span>
        <input type="text" name="placa" value="<?=$placa?>" class="form-control" placeholder="Ej. ABC-123 / BUS 10">
      </div>
    </div>

    <!-- Estado etiqueta -->
    <div class="filter-field">
      <label class="field-label">Estado etiqueta</label>
      <div class="input-wrap">
        <span class="field-ico"><i class="bi bi-tags"></i></span>
        <input type="text" name="estadoetiq" value="<?=$estadoetiq?>" class="form-control" placeholder="CONFORME / estado">
      </div>
    </div>

    <!-- Rango rápido -->
    <div class="filter-field quick-field">
      <label class="field-label">Rangos rápidos</label>
      <div class="quick-list">
        <button type="button" class="quick-chip" onclick="setQuickRange('today')">Hoy</button>
        <button type="button" class="quick-chip" onclick="setQuickRange('7d')">Últimos 7 días</button>
        <button type="button" class="quick-chip" onclick="setQuickRange('30d')">Últimos 30 días</button>
        <button type="button" class="quick-chip" onclick="setQuickRange('ytd')">Año actual</button>
        <button type="button" class="quick-chip" onclick="clearDates()">Sin fechas</button>
      </div>
    </div>
    ...
    <div class="filter-actions">
      ...
      <button type="button" class="btn-filter-secondary" onclick="limpiarFiltrosMov()">
                        ...
                        <tr>
                            ...
                            <td class="d-flex gap-2">
                                <?php if(!empty($r['clm_alm_mov_idNOTA'])): ?>
                                    <button class="btn btn-sm btn-outline-primary btn-note" onclick="verNotaSalida(<?= (int)$r['clm_alm_mov_id']?>)">
