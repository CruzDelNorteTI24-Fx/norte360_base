...
<div class="menu-lateral" id="menuLateral">
  ...
  <ul>
    ...
    <li><a href="tbvistacontratadosent.php">Por Contratar</a></li>
  </ul>
</div>
...
if (data.estado === "4") {
  ...
  <li style="background: #ecf0f1; margin-bottom: 10px; padding: 10px 15px; border-left: 4px solid #27ae60; border-radius: 8px;">
    <strong>🟢 Contrato:</strong> ${data.comentario4 || 'Sin comentario'}
  </li>`;
...
if (data.estado === "5") {
  // Es trabajador, ocultar todo lo de evaluación
...
function contratarTrabajador() {
  ...
  if (!confirm("¿Deseas pasar este entrevistado a la tabla de trabajadores?")) return;

  fetch("../php/contratar_trabajador.php", {
    method: "POST",
    headers: { "Content-Type": "application/x-www-form-urlencoded" },
    body: `id_entrevista=${id}`
  })
  .then(res => res.text())
  .then(data => {
    alert(data);
  ...
  .catch(err => {
    alert("❌ Error al contratar trabajador.");
    console.error(err);
  });
}
  </script>
